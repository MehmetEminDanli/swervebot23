...
public class DriveWithJoysticks implements Command {
    ...
    private final XboxController translation;

    private Rotation2d fieldOrientationZeroOffset = new Rotation2d();

    public DriveWithJoysticks(Drivetrain drivetrain, PoseEstimation poseEstimation, XboxController translation) {
    ...
    public void execute() {
        ...
        double ty = MathUtil.applyDeadband(translation.getLeftX(), 0.1);
        double tx = MathUtil.applyDeadband(translation.getLeftY(), 0.1);
        double r = MathUtil.applyDeadband(translation.getRightX(), 0.1);
